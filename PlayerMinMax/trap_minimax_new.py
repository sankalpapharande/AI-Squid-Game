...
def get_best_diagonal_position(grid, player):
...
def minimax_trap(grid: Grid, depth, player, isMax, alpha, beta, current_trap: tuple):
    ...
    good_traps_against_opponent = get_best_diagonal_position(grid, player)
    ...
    if isMax == 1:
        ...
        for i in good_traps_against_opponent:
            ...
            if hit_probability > 0.9 or depth > MAX_DEPTH - 1:
                expectation = (multiplier - counter) * minimax_trap(grid, depth + 1, player, 0, alpha, beta, i)
            else:
                expectation = (multiplier - counter) * minimax_trap(grid, depth, player, 2, alpha, beta, i)
